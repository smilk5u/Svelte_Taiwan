  ...
  {#if val.seq === currentMenuSubSeq || (currentMenuSubSeq === 0 && i === 0)}
    ...
    {#if category.length > 0}
      <ul class={category.length === 2 ? "tab two_tab" : "tab"}>
        ...
        {/if}
        {#each category as cval, i}
          <!-- svelte-ignore a11y-click-events-have-key-events -->
          {#if val.viewAll == "false" && (currentCetegorySeq === 0 || currentCetegorySeq === 99)}
            <li
              class={i == 0 ? "active" : ""}
              on:click={() => selectedCetegorySeq.set(cval.seq)}
            >
              <a href={cval.url}>{cval.name} </a>
            </li>
          {:else}
            <li
              class={cval.seq === currentCetegorySeq ? "active" : ""}
              on:click={() => selectedCetegorySeq.set(cval.seq)}
            >
              <a href={cval.url}>{cval.name} </a>
            </li>
