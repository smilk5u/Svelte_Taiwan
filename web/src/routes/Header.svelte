  ...
  onMount(async () => {
    ...
    gnbMenuLink.forEach((item) => {
      item.addEventListener("focus", () => {
        GnbOpen();
        header.classList.remove("search_open");
        dimmedMotion(false);
      });
    ...
    /* PC 검색 닫기 */
    closeSearchBtn.addEventListener("click", searchClose);
   ...
   =================================================================================*/
    ...
    window.addEventListener("resize", () => {
      ...
      /* 패널이 열려 있을때 넓이값 유동적으로 변경 */
      if (!searchContMobile.classList.contains("open")) {
        gsap.set(".mobile_panel", {
          right: searchWidth,
        });
      }
    });
