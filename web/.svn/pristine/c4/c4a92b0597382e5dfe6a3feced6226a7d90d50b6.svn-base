<script>
    ...
    onMount(async () => {
        ...
        const categoryl = await fetchData("/api/categoryList?mainSeq=1&subSeq=1", '');
    ...
    function chkOneBox() {
        ...
        let chkItCnt = document
            .querySelectorAll(".chk_it:enabled")
            .length;
        let chkItCnt_chk = document
            .querySelectorAll(".chk_it:checked:enabled")
            .length;
    ...
    <div class="search_bar">
        ...
        <ul class="etc_buttons">
            <li>
                ...
                <select name="category" id="category" on:change|preventDefault={() => page(1)}>
        ...
        <select name="searchOption" id="searchOption" bind:value={searchOption}>
          ...
          />
        <!-- <select name="" id=""> <option value="">제목</option> <option
        value="">작성자</option> <option value="">내용</option> </select> <input
        type="text"/> -->
...
<div class="part">
    <ul class="etc_buttons">
        <li><button on:click={() => goto('/admin/post_add')}>등록</button></li>
        <li>
    ...
    <div class="table_srcoll">
        <form>
            <table>
                ...
                <thead>
                    <tr>
                        <th ><input type="checkbox" name="chk_it" id="chkAll" on:click={chkAllBox}/></th >
                        <th>no</th>
                        <th>카테고리</th>
                        <th>제목</th>
                        <th>글쓴이</th>
  ...
  <nav class="noselect">
    <ul class="pagination justify-content-center">
      <li class="page-item {data.hasPreviousPage ? '' : 'disabled'}" >
        <a class="page-link" on:click={() => page(data.prePage)} aria-label="Previous" >
          <span aria-hidden="true">이전</span>
        </a>
      </li>
      {#each navigatepageNums as pn}
      <li class="page-item {pn == data.pageNum ? 'active' : ''}" >
        <a class="page-link" on:click={() => page(pn)} >{pn}</a>
      </li>
      {/each}
      <li class="page-item {data.hasNextPage ? '' : 'disabled'}" >
        <a class="page-link" on:click={() => page(data.nextPage)} aria-label="Next">
